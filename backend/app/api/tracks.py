...
def list_tracks(playlist_id: int):
    ...
    if playlist is None:
        raise errors.NotFoundError("playlist_id", playlist_id)
...
def get_track(playlist_id: int, track_id: int):
    ...
    if not track:
        raise errors.NotFoundError("track_id", track_id)
    if not track.verify_track_owner(current_user.id):
        raise errors.ForbiddenError("track")
...
def update_track(playlist_id: int, track_id: int, json: TrackPatch):
    ...
    if not track:
        raise errors.NotFoundError(f"track: {track_id} not found")
