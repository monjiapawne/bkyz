...
def test_create_track(client_track):
    client = client_track
    r = client.get("/api/v1/playlists/1/tracks")
...
def test_delete_track(client_track):
    client = client_track
    r = client.delete("/api/v1/playlists/1/tracks/1")
...
def test_progress_track(client_track):
    client = client_track
    r = client.post("/api/v1/playlists/1/tracks/1/progress", json={"position": 49})
    assert r.get_json()["position"] == 50


def test_update_track(client_track):
    client = client_track
    r = client.patch("/api/v1/playlists/1/tracks/1", json={"medium": "pdf"})
